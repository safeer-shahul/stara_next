...
import { Loader2, Home, ShoppingCart } from 'lucide-react';
...
import ShopLayout from '@/app/shop/layout'; // Ensure this path is correct for your layout

type ProductItem = ProductItemDetails;

// Define HomeCategory interface here as well, since this page fetches it directly
interface HomeCategory {
  id: string;
  name: string;
  products: ProductItem[]; // Products array is expected here
}

export default function HomeCategoryProductsPage() {
  ...
  const [error, setError] = useState<string | null>(null); // Re-added the error state
  ...
  const formatSlugToName = useCallback((slug: string): string => {
    return slug
      .replace(/-/g, ' ')
      .split(' ')
  ...
  useEffect(() => {
    ...
    const fetchAndFilterProducts = async () => {
      ...
      const targetCategoryName = formatSlugToName(categorySlug);
      setCategoryDisplayName(targetCategoryName);

      try {
        const allHomeCategories: HomeCategory[] = await apiService.getHomeCategories();

        const foundCategory = allHomeCategories.find(cat => createSlug(cat.name) === categorySlug);

        if (foundCategory) {
          const allCategoryProducts = foundCategory.products || [];
          ...
          const fetchedProductsWithStock: ProductItem[] = paginatedProducts.map((p: any) => {
          ...
          // Calculate if all products for this category have been loaded
          setAllProductsLoaded(products.length + fetchedProductsWithStock.length >= allCategoryProducts.length);

        } else {
          setError(`Category "${targetCategoryName}" not found.`);
          setProducts([]);
          setTotalProducts(0);
          setAllProductsLoaded(true);
        }
      } catch (err) {
        console.error('Failed to fetch and filter home categories:', err);
        setError(`Failed to load products for "${targetCategoryName}". Please try again.`);
        setProducts([]);
        setTotalProducts(0);
        setAllProductsLoaded(true);
      } finally {
  ...
  }, [categorySlug, currentPage, formatSlugToName, createSlug]); // Removed 'products.length' from dependencies

  const loadMoreProducts = useCallback(() => {
    if (!allProductsLoaded && !loadingMore) {
      setCurrentPage(prev => prev + 1);
    }
  }, [allProductsLoaded, loadingMore]);
  ...
  // This check should ideally come after `error` check for clearer messaging
  // This case means categorySlug was null initially and error was set.
  if (!categorySlug) {
    return (
      <>
    ...
    <>
      <div className="container mx-auto p-6 relative">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500">Explore our {categoryDisplayName.toLowerCase()} collection</p>
        </div>
                  ...
                  >
                    ...
                    <div className="mt-4">
                      ...
                      <div className="flex items-center mt-1 gap-1 sm:gap-2">
                        <span className="text-xs sm:text-sm font-semibold">₹{parseFloat(product.product_price).toLocaleString('en-IN')}</span>
                        {parseFloat(product.strike_price) > 0 && (
                          <>
                            <span className="text-xs text-gray-500 line-through">₹{parseFloat(product.strike_price).toLocaleString('en-IN')}</span>
